      ...
      <div className="relative">
        ...
        <ol className="relative space-y-14 before:w-1.5 before:h-[275px] before:bg-[var(--light-silver)] before:absolute before:top-0 before:left-[11px] max-700:before:hidden max-700:space-y-5">
          {steps.map((step, index) => (
            <motion.li
              key={index}
              className="relative flex items-start gap-[31px] text-[20px] leading-[25px] font-medium max-700:text-center max-700:flex-col-reverse max-700:gap-5"
              animate={{
                color:
                  index === activeIndex
                    ? 'var(--jungle-green)'
            ...
            >
              ...
              >
                <div
                  className='w-3 h-3 rounded-full bg-[var(--jungle-green)]'
                ></div>
              </div>
              {step}
            </motion.li>
